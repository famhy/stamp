...
export const StampCanvas = forwardRef<StampCanvasRef, StampCanvasProps>(
  (
    { width, height, shape, onStampComplete, onPointsUpdate, className = "" },
    ref,
  ) => {
    ...
    const clearCanvas = useCallback(() => {
      ...
      if (!ctx) return;
      // width/height are logical CSS px — correct because ctx is scaled by DPR
    ...
    const drawAutoStamp = useCallback(
      (
        ctx: CanvasRenderingContext2D,
        x: number,
        y: number,
      ...
      (
        ...
        if ("touches" in e) {
          ...
          Array.from(e.touches).forEach((touch) => {
            ...
            const pressure =
              (touch as any).force || (touch as any).pressure || 1;
      ...
      (
        ...
        if (currentPoints.length > 0) {
          setContactPoints((prev) => {
            ...
            const novelPoints = currentPoints.filter(
              (p) =>
                !last ||
                Math.abs(last.x - p.x) > 2 ||
                Math.abs(last.y - p.y) > 2,
            );
    ...
    useEffect(() => {
      ...
      // Set buffer to physical pixels (logical size × DPR)
      ...
      // Default styles — set after scale
      ctx.fillStyle = "#000000";
      ctx.strokeStyle = "#000000";
      ctx.lineWidth = 4;
      ctx.lineCap = "round";
